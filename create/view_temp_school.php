...
$PAGE->set_url('/create/view_temp_school.php');
$PAGE->set_pagelayout('standard');
$PAGE->set_title('View Temp School');
echo $OUTPUT->header();
...
echo "Temp School From Excel With Limit";
echo "<table class='table table-striped' style='width:40%;margin-right: 20px;width: 40%;float:left;'>";
foreach($result as $key => $value )
{
	echo '<tr><td>'.$value->id.'</td><td>'.$value->uid.'</td><td>'.$value->email.'</td></tr>';
}
echo "</table>";
}
echo "Temp School From Excel";
$result =$DB->get_records('temp_school_ph2_nov');
